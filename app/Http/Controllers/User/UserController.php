...
class UserController extends Controller
{
    ...
    public function eventStore(Request $request)
    {
        ...
        if ($request->payment_type == 'offline') {
            ...
            $objEventParticipants->save();
            $objUser->notify(new RegisterConfirmation($arrMixExtraData));
            return redirect('upload/receipt/' . $objEventParticipants->id)->with('message',
                'To confirm your registration. Please check your email.');
        }
    ...
    {
        $id = auth()->user()->id;
        $objProfile = Profile::where('user_id', $id)->first();
